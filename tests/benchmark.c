                      ...
                      const bench_opts_t *opts)
{
    ...
    /* 13. AVX-512 Stage 5 OMP (multi-thread) */
    if (opts->kernel_mask & KERNEL_AVX512_S5_OMP) {
        TIME_KERNEL(1, tsmm_avx512_s5_omp(layout, m, n, k, A, B, C,
                                           opts->Ti, opts->Tj, opts->Tk,
                                           opts->num_threads));
        bench_and_report("avx512_s5_omp", shape, layout, min_time_s, opts,
                         have_baseline ? &p_naive_baseline : NULL);
    }

    /* 14. Optimal (auto-select best kernel + tiles from tsmm_tune.h) */
