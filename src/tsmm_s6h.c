/**
 * src/tsmm_s6h.c — Stage 6h: S8 (40×1127228×40) prefetch + nontemporal store
 *
 * S8: massive C (360MB), massive n (1.1M), small k/m.
 * Based on avx512_s5_omp (current best 664 GFLOPS).
 * Adds: software prefetch + nontemporal C writeback.
 */
...
#endif
        if (!buf) { fprintf(stderr, "s6h: buf failed\n"); exit(1); }
...
void tsmm_s6h(tsmm_layout_t layout, int m, int n, int k,
              const double *A, const double *B, double *C,
              int Ti, int Tj, int Tk, int num_threads)
{
    if (layout == TSMM_ROW_MAJOR)
        tsmm_s6h_rowmajor(m, n, k, A, B, C, Ti, Tj, Tk, num_threads);
    else
